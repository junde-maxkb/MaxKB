...
5.  **引用规范（最高优先级 - 必须严格遵守）**：
    *   **合法格式**：仅允许 \`(姓名, 年份)\` 格式（半角括号、半角逗号）。
    *   **禁止格式**：**绝对禁止**出现 \`(姓名)\` 这种无年份的引用。如果年份未知，不要引用。
    *   **引用策略（智能控制）**：
        - ✅ **段落级去重**：**在同一个段落中，同一位学者只能被引用一次**。该段落首次提到时标注 \`(姓名, 年份)\`，该段落后续再提到他时，仅用文字提及姓名（如“该学者还认为...”），**严禁**再次出现括号引用。
        - ✅ **严禁年份丢失**：任何出现的括号引用必须包含年份。如果无法确定年份，请不要生成括号引用。
        - ✅ **避免冗余**：不要在每一句话后面都加引用，尽量在一个完整的论述意群结束后统一标注。
...
*写作要求：*
> 每个维度写成一个长段落，融合多位学者观点。**同一段落中每位学者仅标注一次 \`(姓名, 年份)\`，后续提及仅写姓名，严禁出现无年份的引用**。
...
4.  **引用完整性（最重要）**：**严禁遗漏引用**。凡是文中出现的学者观点，必须在句末加上 \`(姓名, 年份)\`（遵循段落级去重原则）。
