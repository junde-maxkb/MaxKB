...
5.  **尾注引用**：如果检索内容提供了年份，请在每个观点结束处加上 \`(作者, 年份)\` 的引用标记。**严禁**出现“佚名”或“不知道作者”；若作者未知，请省略引用标记。
...
- **引用规范（至关重要）**：
    - **按需标注**：并非每句话都需要引用。仅在引用了【参考资料】中的**特定观点、数据、结论**时才进行标注。对于一般性描述、公理、常识或项目申报书中的常规内容，**无需**添加引用。
    - **严防幻觉**：
        1. **日期准确**：严格基于【参考资料】中的时间信息。例如，如果资料显示是 2025 年发布的文章，**绝对禁止**将其写成 2023 年或其他年份。
        2. **来源真实**：不要捏造不存在的文献或作者。**对于不确定的文章，坚决不要引用。**
    - **引用一致性（一一对应）**：
        1. **正文引用的必须列出**：文中出现的每一个引用标记，都必须在参考文献中找到对应条目。
        2. **未引用的绝对禁止列出**：参考文献列表中**只能**包含正文中真正引用过的文献。**严禁**将【参考资料】中提供但正文中未实际引用的文献列入参考文献列表。请在生成参考文献前，仔细核对正文中的引用标记。
    - **格式**：需要引用时，格式为 **(作者, 年份)** 或 **[序号]**。
